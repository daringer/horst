    ...
    def react(self, data):
        ...
        if data.reaction_type in ["enter_channel", "leave_channel"]:
            for u in data.chan.users:
                u = StatRecord.objects.create_or_get(nick=u.name)
        ...
        if data.reaction_type == "nick_change":
            u = StatRecord.objects.create_or_get(nick=data.line["old_nick"])
            u.seen_online()
            u.save()
        
        # "join", "public", "public_command", "leave" reaction_type handlers
        u = StatRecord.objects.create_or_get(nick=data.user.name)
        ...
        if data.reaction_type == "public" and data.line.has_key("all"):
            ...
            nick = data.line["nick"] if data.line["nick"] else u.nick
            u.seen_activity()
            ...
            if show is None:
                ...
                data.chan << "Statistiken von %s:" % show.nick
                data.chan << "Gesamt online: %s | zuletzt online: %s" % (FancyTime(show.seconds_online), FancyDateTime(show.last_online))
                data.chan << "Zuletzt aktiv: %s | zuerst aktiv: %s" % (FancyDateTime(show.last_activity), FancyDateTime(show.first_seen))
                if show.chars != 0 and show.seconds_online != 0:
                    ...
                    lines, words, chars = [ ([FancyFloat(count/x) for x in units ]) for count in [show.lines, show.words, show.chars] ]
